...
{
    grabContext.chunkBufferLength = 4 * [self width] * [self height];	
...
void yuv_to_rgb_f(UInt8 y, UInt8 u, UInt8 v, Float32 * r, Float32 * g, Float32 * b)
{
    *r = (Float32)y + 1.13983 * ((Float32)v - 128.0);
    *g = (Float32)y - 0.39465 * ((Float32)u - 128) - 0.58060 * ((Float32)v - 128);
    *b =  (Float32)y + 2.03211 * ((Float32)u - 128);
}
//
...
{
    //buffer->buffer = nextImageBuffer;
    memcpy(nextImageBuffer, buffer->buffer, buffer->numBytes);
    return YES;
    ...
    for (row = 0; row < rawHeight; row++) 
    {
        ...
        //memcpy(out, ptr + row * nextImageBufferRowBytes, nextImageBufferRowBytes);
